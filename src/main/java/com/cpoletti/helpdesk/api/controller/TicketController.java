package com.cpoletti.helpdesk.api.controller;
...
import com.cpoletti.helpdesk.api.dto.Summary;
...
import com.cpoletti.helpdesk.api.enums.StatusEnum;
import com.cpoletti.helpdesk.api.response.Response;
...
public class TicketController {
	...
	private void validateCreateTicket(Ticket ticket, BindingResult result)
	{
		...
		{
			result.addError(new ObjectError("Title", "Title not informed"));
		}
	}
	...
	private void validateUpdateTicket(Ticket ticket, BindingResult result)
	{
		...
		{
			result.addError(new ObjectError("Title", "Id not informed"));
		}
		...
		try {
			...
			if (number > 0){
				...
				if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)){
					...
					tickets = ticketService.findByParametersAndCurrenteUser(page, count, title, status, priority, userRequest.getId());
				}
	...
	private void validateChangeStatus(String id, String status, BindingResult result)
	{
		...
		{
			result.addError(new ObjectError("ChangeStatus", "Id not informed"));
		}
		
		if (status  == null || status.equals(""))
		{
			result.addError(new ObjectError("ChangeStatus", "Status not informed"));
		}
	}
